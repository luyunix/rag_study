...
标注成本；视频同时说明，课程当时使用的部分指标仍需要人工标准答案，因为有参考
答案时通常评得更准确。
...
> 版本边界：下面的 `question / answer / contexts / ground_truth` 是视频和 Ragas
> 0.1.x 文档使用的字段名。Ragas 后续版本引入了 `SingleTurnSample` 以及
> `user_input / response / retrieved_contexts / reference` 等命名，并继续调整 API。
> 学指标含义时沿用视频字段；复现代码时必须查看项目锁定版本，不能把两套接口混用。
...
问题是为了覆盖答案里的多个事实。课程画面中的提示词还会让模型标记答案是否
含糊；这里应把它理解为该版本用于惩罚或排除非承诺式回答的实现细节，不能据此
推断所有 Ragas 版本都按完全相同的方式过滤问题。
